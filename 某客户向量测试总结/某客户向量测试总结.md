...
2.   安装配置本地嵌入模型
     ...
     -   方案一：安装kbot核心组建，新版kbot有向量嵌入REST API，安装配置[参见链接](https://github.com/minqiaowang/my-documents/blob/main/kbot-install-guide/Kbot2.0-install-guide-kbot.md)。kbot运行后提供REST API接口。如果GPU资源有富裕，可以多启动几个kbot进程在不同的端口，如（8899，8910，8912等等），REST API接口如下：
