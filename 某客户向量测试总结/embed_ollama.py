...
async def call_text_embedding(data_batch: dict):
    ...
    for record in records:
        try:
            data = {"prompt": record[config("origin_column")], "model": "bge-m3"}
            # 发送 GET 请求
            ...
            if response.status_code == 200:
                ...
                if isinstance(response.json(), dict):
                    # if response.json()["code"] == 200:
                        # record[config("vector_column")] = np.array(response.json()["embedding"].replace("[","").replace("]","").split(",")).astype(np.float32)
